...
- Keep the Vercel project identity `lightfast-www`, its `apps/www` root, and standalone ownership of `lightfast.ai` and `www.lightfast.ai` intact; preserve the `www.lightfast.ai` → `lightfast.ai` HTTP 307 redirect and unrelated DNS, certificates, domains, projects, deployments, environment records, and provider resources.
